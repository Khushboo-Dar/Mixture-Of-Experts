...
def main():
    ...
    if args.path_to_partition_indices:
        with open('data/matminer/saved_partition_indices/task_partition_indices_seed0_correctedExptEf.json', 'r') as f:
    ...
    'dataset': dataset,
    'mean_absolute_deviation': df[dataset_info['target_name']].apply(lambda x: abs(x - df[dataset_info['target_name']].median())).mean(),
  ...
  # initialize model
    ...
    for epoch in range(args.start_epoch, args.epochs):  # default: 1000 epochs
        ...
        # remember the best mae_eror and save 
        if args.task == 'regression':
        ...
        if is_best:
            save_checkpoint({
                ...
                'args': vars(args)},
                is_best, filename=filename_prefix + '_.pth.tar',
...
def initialize_kwargs(**cgcnn_kwargs):
    ...
    **Modified from matminer.featurizers.structure's CGCNNFeaturizer class
    ...
    dataloader_kwargs = \
        {"batch_size": cgcnn_kwargs.get("batch_size", 128),
