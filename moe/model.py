...
# Add at the top of model.py if not already there
from cgcnn.model import CrystalGraphConvNet  # Import the actual model class

def get_extractor(extractor_path, layer_to_extract_from, num_layers_to_unfreeze=1):
    try:
        # Load checkpoint to CPU first
        checkpoint = torch.load(extractor_path, map_location='cpu')
        
        # Initialize model on CPU with the same parameters as stored in checkpoint
        # You might need to adjust these parameters based on your checkpoint
        model = CrystalGraphConvNet(orig_atom_fea_len=checkpoint['model_args']['orig_atom_fea_len'],
                                  atom_fea_len=checkpoint['model_args']['atom_fea_len'],
                                  n_conv=checkpoint['model_args']['n_conv'],
                                  h_fea_len=checkpoint['model_args']['h_fea_len'],
                                  n_l=checkpoint['model_args']['n_l'])
        
        # Load the state dict
        model.load_state_dict(checkpoint['state_dict'])
        
        # Move to GPU if available and enough memory
        if torch.cuda.is_available():
            try:
                model = model.cuda()
            except RuntimeError:
                print("Warning: Not enough GPU memory, using CPU instead")
                model = model.cpu()
